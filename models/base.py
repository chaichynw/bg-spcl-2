...
def get_model(args):
    ...
    if model_name == 'EGNet':
        model = Net(model_name=args.model, 
                    ...
                    use_EEGGram=True,
                    mask_ratio=0.3)
    elif model_name in ['ShallowConvNet', 'DeepConvNet', 'EEGNet', 'EEGChannelNet',
                        'SMCEEGNet', 'SMCShallowConvNet', 'TSception', 'SMCNet', 'EEGTransformer',
                        'MShallowConvNet', 'EEGTCNet']:
        model = Net(model_name=args.model,
                    num_classes=args.num_classes,
                    num_channels=args.num_channels,
                    temporal_size=args.temporal_size,
                    sampling_rate=args.sampling_rate)
    elif model_name == 'HATCN':
        model = Net(model_name=args.model,
                    num_classes=args.num_classes,
                    num_channels=args.num_channels,
                    temporal_size=args.temporal_size,
